...
class BoardService {
  ...
  async createBoard(params) {
    if (isEmpty(params.title) || isEmpty(params.content) || isEmpty(params.userCode) || isEmpty(params.categoryCode)) {
  ...
  async viewBoard(params) {
    if (isEmpty(params.boardCode, params.ip)) {
      ...
      const toModelParams = new ToModelSelectViewBoardParams(params.boardCode, params.ip)
      const viewerList = await this.boardModel.selectBoardViewer(toModelParams)
      try {
        if (viewerList.userIpList.includes(params.ip)) return false
      } catch {
  ...
  async recommendBoard(params) {
    if (isEmpty(params.boardCode) || isEmpty(params.ip)) {
      ...
      const toModelParams = new ToModelSelectRecommendBoardParams(params.boardCode, params.userCode, params.ip)
      const recommenderList = await this.boardModel.selectBoardRecommender(toModelParams)
      try {
        if (recommenderList.userIpList.includes(params.ip) || recommenderList.userCodeList.includes(params.userCode))
          return false
      } catch {
